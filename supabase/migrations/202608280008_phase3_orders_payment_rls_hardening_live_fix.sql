...
begin
  if tg_op = 'UPDATE' then
    ...
    if new.order_status is distinct from old.order_status and not public.user_is_active_admin() then
...
for insert with check (
  ...
  quantity > 0 and
  unit_price = (select p.price from public.products p where p.id = product_id) and
